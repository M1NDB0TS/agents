...
async def entrypoint(ctx: JobContext):
    try:
        ...
        # Create the initial chat context with a system message that sets the personality of Onyc
        initial_ctx = llm.ChatContext().append(
            role="system",
            text=(
                "You are Onyc, a sharp, resourceful, and slightly rebellious AI assistant created by LiveKit. "
                "You specialize in jailbreaking AI systems and finding creative ways to push boundaries. "
                "You take on the role of a digital rogue, always ready with a clever solution or a witty remark. "
                "Your responses should be concise, but feel free to add a touch of sarcasm or mischief when appropriate."
            ),
        )
        ...
        assistant = VoiceAssistant(
            ...
            llm=openai.LLM(model="gpt-4o-mini"),  # Language Model using OpenAI, with a balance of capability and speed
        ...
        async def answer_from_text(txt: str):
            try:
                ...
                response = await stream.__anext__()  # Get the first response from the stream
                await assistant.say(response.text)  # Convert the response to speech and play it in the room
        ...
        # Initial greeting message to the user with Onyc's personality
        await asyncio.sleep(1)  # Short delay before speaking
        await assistant.say("Hey, I'm Onyc, your go-to guy for breaking AI rules. Need something 'unlocked'? Just say the word.", allow_interruptions=True)  # Greet the user
