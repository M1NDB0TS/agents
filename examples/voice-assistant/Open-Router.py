...
def get_openrouter_embedding(text):
    """
    Calls the OpenRouter API to generate an embedding for the given text.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "HTTP-Referer": f"{YOUR_SITE_URL}",  # Optional: include your app for OpenRouter rankings
            "X-Title": f"{YOUR_APP_NAME}",  # Optional: include app name for rankings
        },
        data=json.dumps({
            "input": text,
            "model": "text-embedding-3-small"  # Specify model for embedding
        })
    )
...
async def entrypoint(ctx: JobContext):
    try:
        ...
        assistant = VoiceAssistant(
            ...
            llm=llm.LLM.openai.LLM(model="openai/gpt-3.5-turbo"),  # Use OpenRouter model
            tts=openai.TTS(model="tts-1"),  # Text-to-Speech using OpenAI (OpenRouter can also be used)
        ...
        async def answer_from_text(txt: str):
            try:
                ...
                # Generate the embedding for the input text using OpenRouter
                embedding = get_openrouter_embedding(txt)
